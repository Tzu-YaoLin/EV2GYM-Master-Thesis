...
class Renderer():
    ...
    def render(self):
        ...
        counter = 0
        for i in range(self.env.cs):  # 可以簡化 range(0, X) 為 range(X)
            for j in range(self.env.charging_stations[i].n_ports):
                ev_connected = self.env.charging_stations[i].evs_connected[j]
                ev_label = "ev_" + str(i) + "_" + str(j)
                charger_label = "charger_" + str(i)
        
                if ev_connected is None:
                    self.ev_list[counter].set_visible(False)
